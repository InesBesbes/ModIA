...
Interprétation : on rejette l'hypothèse de nullité ! récupérer de l'information à travers des deux variables
...
Réponse : estimations theta chapeau obs / résidus : vecteur de taille n = epsilon i chapeau observés (yi -Ychapeau i obs) / ychapeau i obs
...
ggplot(ozone, aes(T12, maxO3))+
    ...
    xlab("T12")+  ylab("maxO3")
```
Zone en gris : Intervale de confiance de X_0Theta 
...
```
Premier graphique : résidus observés en fonction des valeurs ajustées. reverifier les hyp : erreurs centres indep et de meme variance : courbe bleu / numéro indiv qui sortent de la tendance : stratégie les enlever et voir si ca ajuste le modele
Deuxieme graph : plus c gaussien, plus les pts vont coller à la bissectrice 
3ème graph : Distance à quel point unindividu peut influencer la valeur de Theta chapeau : plus c haut, plus l'individu qd c utilise pilote l'estimation de theta chapeau
...
ggplot(new_df, aes(T12, maxO3))+
    ...
    ylab("maxO3")
```
sigma chapeau carré super elevé et avec le +1 on a bien élargi l'int de prédiction 
...
ggplot(data=df1[-1,],aes(x=intp, y=slope))+
  ...
  geom_point(data=df1[1,], aes(x=intp, y=slope), 
               pch=3)
```
rouge : croisement des deux intervalles de confiance et c centrée en theta chap obs
noir : région de confiance construite ac fisher / ellipse noir : proba que theta tombe la bas est de nv 95% / à savoir retrouver (région de confiance) abscisse : theta 0 et ordonne theta 1
...
$$
Y = \theta_0 + \theta_1*x_1+...+\theta_n*x_n+\epsilon
$$
...
```
savoir expliquer forward/backward/ et aic/bic...
...
```
il faut regarder que les sous modeles sont inclus les uns dans les autres !!! sinon on peut pas faire de test de fisher pr les comparer 
...
ggplotly(gmse)
```
plus lambda augmente plus on enleve les parametres 
pointilles = intervalle de confiance 
...
ggplotly(g3)
```
Lasso : chemins vont s'effondrer au fur et à mesure T12 et maxO3v restent le plus longtemps
il faut trouver le lambda où on coupe 
lambda = 0 = on a enleve les variables du modele
...
g3
```
soit lambda qui rend minimul mse soit celui avec ecart type de 1
1er lambda : on conserve plus de variables 
...
ggplotly(g4)
```
lasso = pénalité l1 qui va forcer les coefficients à aller s'annuler
