...
velibacp
```
si on prend tout le jeu on est à 24*7
class distance somme sur toutes les variables donc on va cumuler toutezs les fluctuations
quand on a trop de variables il faut faire une réduction des données
...
quand on agrège on perd en inertie intra 
démontrer la balance 

quels outils on utilise pour comparer deux clustering sur un mm jeu de données
...
```
nbre de paramètres : le nbre de var intervient 
matrice de variance covariance ça coute p(p+1) paramètres
