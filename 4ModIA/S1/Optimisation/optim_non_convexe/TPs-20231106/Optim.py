...
def ls_wolfe(x, function, step, descent,f,df):
    ...
    while ((f2 > f + e1*step2*np.dot(df,descent)) | (e2*np.dot(df,descent)>np.dot(df2,descent))):
...
class BFGS():
    ...
    def push(self, x, grad):
        if self.last_iter != []:
            ...
            if rho>0:
                if len(self.stock)<=self.nbre_max:
    ...
    def get(self, grad):
        if self.stock != []:
            ...
            for i in range(len(self.stock)-1,-1, -1):
                alpha[i] = self.stock[i][2]*np.dot(self.stock[i][0], g)
                g = g - alpha[i] * self.stock[i][1]
            gamma = np.dot(self.stock[0][0],self.stock[0][1]) /np.dot(self.stock[0][1],self.stock[0][1])
            r = gamma * g

            for i in range(len(self.stock)):
                beta[i] = self.stock[i][2]*np.dot(self.stock[i][1],r)
                r = r + (alpha[i] - beta[i])*self.stock[i][0]
            return r
