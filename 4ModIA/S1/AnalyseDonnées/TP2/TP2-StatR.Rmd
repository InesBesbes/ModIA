...
sigma2est=NULL
for (i in 1:length(n)){
  x = rnorm(n[i],5,2)              # echantillon à simuler
...
int.conf.moy1 <- function(x,niv.conf,sigma2){
  alpha = 1-niv.conf
  IC = mean(x) + c(-1,1) * qnorm(1-alpha/2) *sqrt(var(x)/length(x))
  return(IC)
}
```
...
niv.conf = c(0.05,0.1,0.25,0.5)
for (i in 1:length(n)){
  for (j in 1:length(niv.conf)){
    x= rnorm(n[i], 5,2)
    IC=int.conf.moy1(x, niv.conf[j], var(x))
...
int.conf.moy2 <- function(x,niv.conf){
  ...
  IC = mean(x) + c(-1,1) * qt(1-alpha/2,length(x)-1) * sqrt(S2**2/length(x))
  return(IC)
}
```
...
estim.prop.test.moy1 <-function(n=1000,m=5,sigma2=4,m0=5,alpha=0.05,K=100,alternative="greater"){
  nb.rejets=0
  for(k in 1:K){
      x = rnorm(n, m0,sqrt(sigma2)) # echantillon simule
      pval = test.moy1(x, sigma2, m0, alternative) # en utilisant test.moy1
...
for (i in alphas) {
  dim(seq(5,10,i))
  par(new =T)
  plot(seq(5,10,0.01),puiss.test.moy.1(1000,4,5,i,10), type = "l")
}
```
