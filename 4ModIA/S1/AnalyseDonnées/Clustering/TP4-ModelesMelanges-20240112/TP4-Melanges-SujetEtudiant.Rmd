...
gprobapost
```
si individu à -4 ca veut dire qu'on va le mettre classe 1
hesitation entre rouge et verte : 2 proba condi à 0.5
plus on se rapproche, plus la zone d'incertitude et chevauchement va augmenter 
...
```
plus boxplot serres et proches de 1, plus les classes sont séparées
trainée de point boxplot milieu à cause du chevauchement
...
**Question  :** A l'aide de la fonction `Mclust()`, estimez les paramètres des mélanges gaussiens considérés. Vous pouvez consulter l'aide de la fonction `` pour le choix des formes des mélanges.
...
summary(resBICdiag)
```
EM : initialisation aléatoire!
evi : meme volume pas mm forme
bic a besoin de bvp de classes pr pouvoir ajuster
...
proba : ca colle bcp mieux 
pdv estimation de densité : c pourri pas bien estimé
...
# Effectifs par classe
```
kmeans : imposer forme sphérique 
...
## graphique à faire
#barplotClus(resmixmod@results, zoo, 5)
resbic = Mclust(zoo)
...
## A completer
```
Var qualt : travailler sur les coordonnees des var après réduction de dimension