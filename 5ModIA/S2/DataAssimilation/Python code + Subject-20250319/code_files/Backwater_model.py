...
def J_res(model, col, domain):
    ...
    residu = h_prime + (b_prime + j)/(1-Fr2)

    return (torch.norm(residu, p=2)**2)/len(domain)

def J_obs(model, obs):
    """
    Loss function for observations discrepancy. 
    """
    hpred = model(obs.data[:,0].view(-1,1))
    hobs = obs.data[:,1].view(-1,1)
    return (torch.norm(hobs-hpred)**2)/obs.N_obs
