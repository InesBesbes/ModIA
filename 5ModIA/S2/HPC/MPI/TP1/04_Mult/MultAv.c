...
int main(int argc, char *argv[])
{
  ...
  if (my_rank == 0)
  {
    ...
    {
      v[i] = i;
      printf("Process [%d], v[%d] = %f\n", my_rank, i, v[i]);
    }
  }

  // Communication of v / doit être connu de tous donc broadcast : y a que le 0 qui lit les données puis il va partager les données donc faire un broadast. S'il nyavait pas le if my rank on aurait pas eu besoin de faire le broadcast pcq v sera connu de tous 
