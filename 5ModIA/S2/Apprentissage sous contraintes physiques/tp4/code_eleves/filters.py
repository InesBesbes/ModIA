...
class Lin2d(nn.Module):
    ...
    def __init__(self, x_dim, N, dt, init,
        ...
        self.M[1, 1] = np.cos(theta)
        self.x0 = me.get_x0(l2d.b_size, x_dim, sigma=l2d.sigma0)
...
class EDO(nn.Module):
    ...
    def edo(self, x):
        ...
        # TODO bonus
        v = torch.cat(
            [torch.roll(x.unsqueeze(1), -1, 2) for i in self.window], 1)
        v = torch.transpose(v, 1, 2)
        v_flat = v.reshape(-1, self.diameter)
        dx = torch.nn.functional.bilinear(v_flat, v_flat, self.A) + torch.nn.functional.linear(v_flat, self.b, self.c)      
...
class FcZero(nn.Module):
    ...
    def __init__(self, dim, deep, activation_classname):
        ...
        self.alphas = nn.Parameter(torch.zeros(deep), requires_grad=True) #le rendre apprenable
...
class Gaussian(nn.Module):
    ...
    def __init__(self, x_dim, vec, inds):
        ...
        diaga = torch.max(minexp, torch.min(diaga, maxexp))
        ...
        # self.cov_inv_ = torch.linalg.solve_triangular(self.Lambda, torch.eye(x_dim).unsqueeze(0).expand(bs,x_dim,x_dim), upper=False)
        self.cov_inv_ = torch.linalg.solve_triangular(self.Lambda, torch.eye(x_dim, device=vec.device).expand(bs,x_dim,x_dim), upper=False)
    ...
    def log_prob(self, x):
        ...
        #     term_1 = torch.mm(mean_diff.unsqueeze(0), cov_inv)
        #     term_1 = torch.mm(term_1, mean_diff.unsqueeze(1))
        #     term_1 = term_1/2
