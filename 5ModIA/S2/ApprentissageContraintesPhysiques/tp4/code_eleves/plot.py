...
model = torch.load("lin2d_exp/test_scores.pt") 
print(model)
...
plt.scatter(xtps[:,0,0], xtps[:,0,1],label="obs")
plt.scatter(yts[:,0,0], yts[:,0,1],label="solution modèle (train)")
plt.scatter(yts[:,0,0], yts[:,0,1],label="solution modèle (test)")
