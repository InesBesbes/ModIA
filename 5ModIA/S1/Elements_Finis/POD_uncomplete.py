...
U_pred = np.load('/home/besbesines/Documents/5ModIA/Elements_Finis/POD-NN-practical/Uh_POD_NN.npy')
u_pred = Function(V)

u_pred.vector().set_local(U_pred)

p=plot(uh, title="The FE solution")#,mode='color',vmin=1.0, vmax=1.0000040)
p.set_cmap("rainbow"); plt.colorbar(p); plt.show()

p=plot(U_pred, title="The predicted solution")#,mode='color',vmin=1.0, vmax=1.0000040)
p.set_cmap("rainbow"); plt.colorbar(p); plt.show()
